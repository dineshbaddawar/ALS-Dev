({
    ...
    parentComponentEvent:function(component, event, helper) {
        ...
        action.setCallback(this,function(response){  
            ...
            if(state ==="SUCCESS"){
                ...
                component.set("v.totalTargetAchieved" , parseInt(totalTargetAchived)/10000000 +"cr"    ); 
    ...
    handleYearlyGoal: function (component, event,helper) {
        debugger;
        var action = component.get("c.getYearlyGoal");
        action.setParams({
            fiscalId : component.get("v.auraFiscalId"),
            userId : component.get("v.auraUserId")
        })
         action.setCallback(this,function(response){  
            var state = response.getState();
            if(state ==="SUCCESS"){
                var data = response.getReturnValue();
                if(data){                
                component.set("v.yearlyGoallistRec",data );
                }
            }
        });
        $A.enqueueAction(action);
    ...
    handleLWCEventFiscal: function(component, event, helper) {
        debugger;
       var fiscalIdFromLWC = event.getParam('value');        
       // console.log(userIdFromLWC);
        component.set("v.auraFiscalId", fiscalIdFromLWC);
    },
